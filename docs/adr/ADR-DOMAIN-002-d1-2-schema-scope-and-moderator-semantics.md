...
Delivery D1.2 scope and Moderator semantics.

> **Accepted decision:** This ADR records the accepted Delivery D1.2 scope and Moderator semantics decision. It does not authorize Delivery D1.2 implementation, and Delivery D1.2 remains frozen. The accepted baselines and accepted ADRs remain authoritative. This ADR does not modify or supersede ADR-DOMAIN-001.

## Context and problem

The exact first MVP includes Scientist, SA, and Moderator roles in one standalone room. ADR-DOMAIN-001 defines the Delivery D1 domain actor values as `USER`, `SCIENTIST`, `SA`, and `SYSTEM`. The North Star Vision and Architecture Impact Assessment identify a vocabulary clarification: Moderator is visible in the first-MVP user experience, but the current four `ActorKind` values do not name a separate Moderator actor.

This ADR records how a future, separately approved Delivery D1.2 schema foundation expresses that exact first-MVP distinction without expanding the domain actor vocabulary, creating a generic role platform, or treating a user-facing role label as human authority.
...
- GitHub Issue #14 records the explicit human acceptance context for this ADR.
- Delivery D1.2 remains frozen.
- The Project Charter, System Design, ADR-GOV-001, and ADR-DOMAIN-001 remain authoritative.
- This ADR must not modify, supersede, or reinterpret ADR-DOMAIN-001.
...
- GitHub Issue #14 records the Lead-SA-verified human acceptance of this ADR.
...
| Concept | Accepted first-MVP meaning | Must remain distinct from |
|---|---|---|
...
Role remains separate from model and provider selection. This ADR selects neither a model nor a provider, and it does not authorize a tool-permission implementation.

### D1.2 scope guardrails

The accepted D1.2 boundary is a strict Pydantic contract-schema foundation for already approved Delivery D1 concepts only. Acceptance of this ADR does not authorize Delivery D1.2 implementation. It may rely on the Pydantic dependency already approved by ADR-DOMAIN-001 for `libs/contracts`; this ADR does not add, remove, or modify dependencies or lockfiles.

The following are explicitly outside the accepted D1.2 scope:
...
- This ADR deliberately does not solve future dynamic roles, temporary specialists, or organizational role policy.
...
- Any attempt to implement more than the strict D1.2 foundation would exceed this accepted scope and require a separate architectural decision and explicit human approval.

## Dependency impact

No dependency impact is authorized. ADR-DOMAIN-001 already approves `pydantic==2.13.4` as a direct dependency of `libs/contracts`; this ADR neither changes that approval nor authorizes any dependency or lockfile change.

## Security impact

This ADR introduces no security implementation. Its accepted conceptual boundary supports least-authority reasoning by keeping Moderator separate from human authority, tool permissions, external action, provider choice, and authentication. Any security, consent, access, or tool-policy design remains outside Delivery D1.2.

## Data and migration impact

No data model, persistence design, migration, retention mechanism, or audit-storage implementation is authorized. Persisted mutation issuer is an accepted contract semantic only; it does not select a database, storage technology, or migration path.

## Operational impact

No runtime orchestration, service behavior, worker behavior, API route, provider integration, queue, or deployment change is authorized. Delivery D1.2 remains frozen pending a separately approved bounded implementation issue and a human-approved plan.

## Validation strategy

For this accepted ADR:

- Verify its status is Accepted everywhere it is indexed or described.
- Verify the accepted actor set remains exactly `USER`, `SCIENTIST`, `SA`, and `SYSTEM`.
- Verify this ADR explicitly excludes adding `MODERATOR` to `ActorKind` and creating a generic role registry.
...
If a later approved implementation issue and plan separately authorize implementation, future contract tests should verify the stated actor values, attribution distinctions, user-only authority guards, and the absence of acceptance or publication behavior in Delivery D1. No implementation or test change is authorized by this ADR.

## Rollback or recovery

If a later accepted decision requires a different boundary, it must be recorded through the applicable human-governed ADR or System Design process. This ADR does not alter ADR-DOMAIN-001.

## Rejected alternatives

This ADR formally rejects Options A, C, and D:
...
These alternatives remain rejected by this accepted ADR.
...
This accepted ADR creates no implementation work. If a later approved implementation issue and plan separately authorize a bounded D1.2 batch, traceability should include:
...
- Reviewers: Human project owner, Lead SA
- Approval date: 2026-07-18
- Approval source: explicit human decision associated with GitHub Issue #14
- ADR acceptance does not authorize Delivery D1.2 implementation
- Delivery D1.2 remains frozen pending a separate approved implementation issue and plan

## Supersession record

None. This ADR does not supersede ADR-DOMAIN-001, any other ADR, or an accepted baseline.
